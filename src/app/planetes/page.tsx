...
const PLANETS = [
    ...
    { name: 'Terre', emoji: '🌍', texture: '/textures/earth.jpg', color: '#60a5fa', type: 'Rocheuse', gravity: 9.81, meanRadius: 6371.0, avgTemp: 288, density: 5.51, distSun: 149.6, moons: 1, orbit: 365, atmosphere: '#60a5fa', description: 'Notre maison ! Seule planète connue à abriter la vie. 71% de sa surface est de l\'eau.', fun: 'La Terre est la seule planète non nommée d\'après un dieu romain.' },
    { name: 'Mars', emoji: '♂', texture: '/textures/mars.jpg', color: '#f87171', type: 'Rocheuse', gravity: 3.72, meanRadius: 3389.5, avgTemp: 210, density: 3.93, distSun: 227.9, moons: 2, orbit: 687, atmosphere: '#ef444430', description: 'La planète rouge. Des rovers l\'explorent. Olympus Mons est le plus grand volcan du système solaire (21 km).', fun: 'Mars a le plus grand canyon du système solaire : Valles Marineris, 4 000 km de long !' },
    ...
    { name: 'Uranus', emoji: '♅', texture: '/textures/uranus.jpg', color: '#67e8f9', type: 'Géante de glace', gravity: 8.69, meanRadius: 25362, avgTemp: 76, density: 1.27, distSun: 2871, moons: 28, orbit: 30687, atmosphere: '#67e8f9', description: 'La planète qui \'roule\' sur le côté. Découverte en 1781. Il y pleut peut-être des diamants.', fun: 'Uranus a une inclinaison de 98° — elle tourne sur le côté comme une toupie !' },
    { name: 'Neptune', emoji: '♆', texture: '/textures/neptune.jpg', color: '#818cf8', type: 'Géante de glace', gravity: 11.15, meanRadius: 24622, avgTemp: 72, density: 1.64, distSun: 4495, moons: 16, orbit: 60190, atmosphere: '#a5b4fc', description: 'La plus lointaine et la plus venteuse. Vents à 2 100 km/h ! Découverte en 1846 grâce aux maths.', fun: 'Neptune a été découverte mathématiquement avant même d\'être observée !' },
]
        ...
        <div className="container" style={{ paddingTop: '3rem', paddingBottom: '6rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem', marginBottom: '2rem' }} className="max-sm:grid-cols-2">
                {PLANETS.map((pl, i) => (
                    <button key={pl.name} onClick={() => setSelected(i)} style={{
                        ...
                        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: 1 }}>{pl.moons} lune{pl.moons !== 1 ? 's' : ''}</div>
                    </button>
                ))}
            </div>
            ...
            <div className="card" style={{ padding: '1.25rem', marginBottom: '2rem' }}>
                ...
                <p style={{ color: 'var(--text-muted)', lineHeight: 1.8, fontSize: '0.875rem' }}>
                    ...
                    Les 4 suivantes sont des <strong style={{ color: 'var(--nebula)' }}>géantes gazeuses ou de glace</strong> (Jupiter, Saturne, Uranus, Neptune).
                    Les modèles 3D utilisent les <strong style={{ color: 'var(--text)' }}>textures officielles de la NASA</strong>.
                </p>
            </div>
                        ...
                        return (
                            <div key={pl.name} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.375rem', cursor: 'pointer' }}
                                ...
                                <div style={{ textAlign: 'center' }}>
                                    ...
                                    <div style={{ fontSize: '0.55rem', color: 'var(--text-muted)' }}>×{xTimes} Terre</div>
                                </div>
                            </div>
                        )
                    })}
                </div>
                <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.62rem', marginTop: '0.25rem' }}>Clique sur une planète pour l&apos;explorer · ×1.0 = Terre (rayon 6 371 km)</p>
            </div>
            ...
            <div className="card" style={{ padding: '1.5rem', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.25rem' }} className="max-sm:grid-cols-1">
                    {[{ v: comp1, set: setComp1 }, { v: comp2, set: setComp2 }].map((c, ci) => (
                        <div key={ci}>
                            <label htmlFor={`planet-compare-${ci}`} style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginBottom: '0.3rem', display: 'block' }}>Planète {ci + 1}</label>
                            <select id={`planet-compare-${ci}`} value={c.v} onChange={e => c.set(parseInt(e.target.value))} style={{
                                ...
                                {PLANETS.map((pl, i) => <option key={i} value={i} style={{ background: 'var(--card)' }}>{pl.emoji} {pl.name}</option>)}
                            </select>
                        </div>
                    ))}
                </div>
                    ...
                    <tbody>
                        {PLANETS.map((pl, i) => (
                            <tr key={pl.name} tabIndex={0} onClick={() => setSelected(i)} onKeyDown={event => {
                                ...
                                <td style={{ padding: '0.625rem 0.875rem', color: 'var(--text-muted)' }}>{pl.type}</td>
                                <td style={{ padding: '0.625rem 0.875rem', color: 'var(--text)' }}>{pl.meanRadius.toLocaleString('fr-FR')}</td>
                                ...
                                <td style={{ padding: '0.625rem 0.875rem', color: 'var(--text)' }}>{pl.orbit.toLocaleString('fr-FR')} j</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <Suspense fallback={<div style={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>Chargement des positions…</div>}>
                    <SolarSystem2D />
                </Suspense>
            </div>
        </div>
