  ...
  return (
    <div className="container prose-page parent-page">
      ...
      <section className="parent-intro card">
        <h2>Comment utiliser SolarScope</h2>
        <ol>
          <li>Choisissez une mission courte plutôt que de tout parcourir.</li>
          <li>Commencez par la question et l’analogie, puis ouvrez les détails seulement si l’enfant est curieux.</li>
          <li>Terminez par le défi ou une discussion ; se tromper au quiz fait partie de l’apprentissage.</li>
        </ol>
      </section>

      <section aria-labelledby="guide-title">
        <div className="parent-guide-heading">
          <h2 id="guide-title" className="parent-section-title">Fiches express</h2>
          <PrintableGuidesButton />
        </div>
        <div className="parent-guide-grid">
          {GUIDES.map(({ title, age, duration, href, goal, prompt }) => (
            <article className="card parent-guide" key={title}>
              ...
              <dl className="parent-guide-details">
                <div className="parent-guide-detail"><dt>Objectif</dt><dd>{goal}</dd></div>
                <div className="parent-guide-detail"><dt>À demander</dt><dd>{prompt}</dd></div>
              </dl>
              <Link href={href} className="parent-guide-link">Ouvrir la mission <span aria-hidden="true">→</span></Link>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="parent-intro card parent-safety">
        <h2>Données, publicité et sécurité</h2>
        <p>Le passeport spatial est enregistré seulement dans le navigateur de l’appareil. SolarScope ne crée pas de profil enfant. Les données scientifiques viennent de sources affichées dans les pages ; leur fraîcheur peut varier et une valeur indisponible doit être comprise comme telle.</p>
        <p>SolarBot rappelle de ne pas partager de nom, école, adresse, téléphone ou e-mail. Pour une information importante, vérifiez toujours la source scientifique liée.</p>
      </section>
    </div>
  )
}
