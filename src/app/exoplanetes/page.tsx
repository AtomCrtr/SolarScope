...
const FAMOUS_EXOPLANETS = [
    ...
    { name: 'TRAPPIST-1e', distance: '40 années-lumière', type: 'Rocheuse', inHabitableZone: true, color: '#60a5fa', description: "C’est l’une des sept planètes rocheuses du système TRAPPIST-1. Sa taille est proche de celle de la Terre.", fun: "Ces sept planètes tournent très près de leur petite étoile, mais certaines reçoivent une quantité d’énergie comparable à la Terre." },
    { name: 'Kepler-452b', distance: '1 400 années-lumière', type: 'Super-Terre', inHabitableZone: true, color: '#a5b4fc', description: "Son année dure 385 jours et son étoile ressemble au Soleil. Elle est toutefois plus grande que la Terre et sa surface reste inconnue.", fun: "Être dans la zone habitable ne suffit pas : il faut aussi connaître l’atmosphère et la présence éventuelle d’eau." },
    ...
    { name: 'WASP-12b', distance: '1 400 années-lumière', type: 'Jupiter chaud', inHabitableZone: false, color: '#f87171', description: "Cette planète passe si près de son étoile qu’elle est déformée et perd peu à peu de la matière.", fun: "Elle renvoie très peu de lumière visible : les observations montrent qu’elle en absorbe presque toute." },
]

const DETECTION_METHODS = [
    { name: 'Transit', emoji: '🌑', usage: 'La plus utilisée', color: '#a5b4fc', description: "Quand la planète passe devant son étoile, la luminosité baisse légèrement. Comme si une mouche passait devant un phare !" },
    ...
    { name: 'Microlentille', emoji: '🔍', usage: 'Rare', color: '#f87171', description: "La gravité de l’étoile et de sa planète déforme la lumière d’une étoile lointaine, comme une loupe." },
]
        ...
        <div className="container" style={{ paddingTop: '3rem', paddingBottom: '6rem' }}>
            ...
            <div className="card" style={{ padding: '1.25rem', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.5rem' }}>
                    {[
                        ...
                        { label: 'Neptune', marker: 'atmosphère épaisse', color: '#60a5fa', desc: 'Un monde souvent composé de gaz et de glaces' },
                        { label: 'Géante gazeuse', marker: 'très grande', color: '#f87171', desc: 'Un monde surtout composé de gaz, comme Jupiter ou Saturne' },
                    ].map(c => (
                        <div key={c.label} style={{ padding: '0.875rem', borderRadius: '0.75rem', background: `${c.color}08`, border: `1px solid ${c.color}25` }}>
                            <div style={{ color: c.color, fontWeight: 700, fontSize: '0.875rem', marginBottom: '0.25rem' }}>{c.label} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>— {c.marker}</span></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '2rem' }}>
                {DETECTION_METHODS.map(m => (
                    <div key={m.name} className="card" style={{ padding: '1.25rem', textAlign: 'center', borderLeft: `4px solid ${m.color}` }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', lineHeight: 1.6 }}>{m.description}</p>
                    </div>
                ))}
            </div>
                ...
                {filtered.map(exo => (
                    <div key={exo.name} className="card" style={{ padding: '1.25rem', borderLeft: `4px solid ${exo.color}` }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                            ...
                            <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', whiteSpace: 'nowrap', marginLeft: '0.5rem' }}>{exo.distance}</span>
                        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                {VIDEOS.map(v => (
                    <div key={v.title} className="card" style={{ padding: '1.25rem' }}>
                        <h3 style={{ color: 'var(--text)', fontWeight: 700, fontSize: '0.875rem', marginBottom: '0.5rem' }}>{v.title}</h3>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.75rem', lineHeight: 1.6 }}>{v.description}</p>
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
                ...
                <ul className="resource-list" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                    {[
                        { label: 'NASA Exoplanet Exploration', url: 'https://science.nasa.gov/exoplanets/', desc: 'Le portail officiel avec visualisations interactives' },
                        ...
                        { label: 'ESO Exoplanètes', url: 'https://www.eso.org/public/france/science/exoplanets/', desc: "Le site de l'Observatoire Européen" },
                    ].map(l => (
                        <li key={l.label}>
